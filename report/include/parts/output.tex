...
The functions can be represented according to two methods as shown in \figref{fig:output:rdd-example}.
...
\bigfig{fig:output:rdd-example}{insert/output}{100_rho5e13m-2_square_3200nm_RDD_d5e-5nm-2_edge_S0_PBC1_output}{Simulation output of \gls{rdd}}

The example shown in \figref{fig:output:rdd-example} can be exported with the following script.
...
\( k \) can be compared to the directing coefficient of the linear part in \gls{r2} in \figref{fig:output:rdd-example}.
...
Compared to the original proposal, \( \langle \gls{dst}^2 \rangle - \langle \gls{dst} \rangle^2 \) has been simplified to \( \delta \gls{dst} \) with \( \delta \) a coefficient quantifying the density fluctuation and \( \ln(\gls{frrvar}/R_1) \ln(\gls{frrvar}/R_2) \) has been simplified to \( (\ln(\gls{frrvar}/R_0))^2 \).
...
\bigfig{fig:fit:example:rdd}{insert/fits/100_rho5e13m-2_square_3200nm_RDD_d5e-5nm-2_edge_S0_PBC1_output_analysis/fits_plot_GUW2}{j1_118nm}{Fit of \gls{guw2} over \gls{rdd} output}
...
An overview of the data generated to conduct the study is presented in section \ref{sec:data}.
The data from the fits in particular are difficult to compare as they are voluminous and specific.
It is proposed here a synthesis of the results of the fits.
...
For this purpose, three approaches are proposed.

\begin{itemize}
  \item Calculate the mean of the values.
  \item Calculate the standard deviation.
  \item Take the value for the closest value of \gls{frrvar} to the mean inter dislocation distance.
\end{itemize}

To simplify the reading of the tables, the density values closest to the real densities have been grayed out.
Sometimes there are several when they are equal.
Not all decimal places are displayed.

\bigskip

Below is an overview of the parameters of the samples of distributions studied.
