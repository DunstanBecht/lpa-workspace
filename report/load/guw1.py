...
for i in range(len(dismodord)):
    for l in range(len(stmlstfit[i][0])):
        ...
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(8, 3.1))
        fig.subplots_adjust(left=0.32, right=0.96, bottom=0.15, wspace=0.33)
        ...
        for k in range(len(readst)):
            for h in [1, 2]:
                ...
                y1 = []
                y2 = []
                for m in range(1, len(fittbl[i_L])):
                ...
                mkr = '-' if h==1 else ':'
                ax1.plot(x, y1, mkr, label=lab, color=col)
                ax2.plot(x, y2, mkr, color=col)
                
                mkr = 'o' if h==1 else '.'
                ax1.plot(x[-1], y1[-1], mkr, color=col)
                ax2.plot(x[-1], y2[-1], mkr, color=col)

                i1 = np.argmin(np.abs(1-x))
                ax1.plot(x[i1], y1[i1], mkr, color=col)
                ax2.plot(x[i1], y2[i1], mkr, color=col)
